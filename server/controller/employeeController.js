...
const addEmployee = async (req, res) => {
    ...
    if (user) {
      return req.status(400).json({success: false, error: "user already registered in employees"});
    }
    ...
    return res.status(500).json({success: false, message: "server error in addin employee"})
...
const updateEmployee = async (req, res) => {
    try {
        ...
        if(!employee) {
          return res.status(404).json({ success: false, error: "employe not found" });
        }
